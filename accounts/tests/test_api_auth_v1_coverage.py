...
class ApiAuthV1CoverageTests(TestCase):
    ...
    def setUp(self):
        """Create a passwordless social user fixture."""
    ...
    def test_social_providers_and_connections_cover_configured_states(self):
        ...
        api_v1._configured_providers.cache_clear()
    ...
    def test_disconnect_social_account_error_and_success_branches(self):
        ...
        last_only = self.User.objects.create_user(
            username="social-last-method",
            email="social-last@example.com",
        )
        last_only.set_unusable_password()
        last_only.save(update_fields=["password"])
        ...
        last_only_headers = {
            "HTTP_AUTHORIZATION": f"Bearer {create_access_token(last_only)}"
        }
        ...
        # Two connections on self.user → can disconnect one of them
        UserSocialAuth.objects.create(
            user=self.user,
            provider="github",
            uid="kept",
    ...
    def test_social_start_and_callback_cover_redirect_branches(self):
        ...
        inactive.save(update_fields=["password"])
    ...
    def test_refresh_logout_me_endpoints_cover_error_branches(self):
        """Auth endpoints should cover invalid token branches."""
    ...
    def test_social_exchange_rejects_inactive_or_merged_users(self):
        ...
        merged.set_unusable_password()
        merged.save(update_fields=["password"])
